...
class plexTV(object):
    ...
    def __init__(self):
        ...
        self.sharedServersURL = self.mainUrl + \
            '/api/servers/' + self.id + '/shared_servers'
        self.token = None
        # Mandentory headers
        self.myHeader = {}
                ...
                for server in servers:
                    if server.get('machineIdentifier') == self.id:
                        if len(sharedUsers.xpath('//SharedServer[@userID=' + user.get('id') + ']/@accessToken')) > 0:
                            ...
                            usrList[usr]['allowCameraUpload'] = user.get(
                                'allowCameraUpload')
                            usrList[usr]['allowChannels'] = user.get(
                                'allowChannels')
                            usrList[usr]['filterAll'] = user.get('filterAll')
                            usrList[usr]['filterMovies'] = user.get(
                                'filterMovies')
                            usrList[usr]['filterMusic'] = user.get(
                                'filterMusic')
                            usrList[usr]['filterPhotos'] = user.get(
                                'filterPhotos')
